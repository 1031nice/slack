...
Single-server WebSocket architecture cannot scale horizontally and has a single point of failure. Need pub/sub mechanism for multi-server message broadcasting.
...
**What we learned:**
- **Evaluate tool combinations, not individual tools**: PostgreSQL + Redis vs PostgreSQL + Kafka, both provide durability
